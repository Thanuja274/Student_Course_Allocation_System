...
export default function StudentDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setPreferences(pR.data.map(p => ({
        course_id:p.course_id, priority_rank:p.priority_rank,
        name:p.name, code:p.code, credits:p.credits, schedule:p.schedule,
      })));
      setResult(rR.data);
  ...
  const addCourse = course => {
    ...
    if (preferences.length>=8) { setError('Maximum 8 preferences'); return; }
    setError('');
    setPreferences(prev=>[...prev,{
      course_id:course.id, name:course.name, code:course.code,
      credits:course.credits, schedule:course.schedule,
      priority_rank:prev.length+1,
    }]);
  };

  const removeCourse = courseId =>
    setPreferences(prev=>prev.filter(p=>p.course_id!==courseId).map((p,i)=>({...p,priority_rank:i+1})));

  const moveUp = i => {
    if (i===0) return;
    setPreferences(prev=>{ const a=[...prev]; [a[i-1],a[i]]=[a[i],a[i-1]]; return a.map((p,j)=>({...p,priority_rank:j+1})); });
  };
  const moveDown = i => {
    setPreferences(prev=>{ if(i===prev.length-1) return prev; const a=[...prev]; [a[i],a[i+1]]=[a[i+1],a[i]]; return a.map((p,j)=>({...p,priority_rank:j+1})); });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post('/preferences', { preferences: preferences.map(p=>({course_id:p.course_id, priority_rank:p.priority_rank})) });
  ...
  const clearAll = async () => {
    ...
    try { await api.delete('/preferences'); setPreferences([]); setMessage('Preferences cleared'); }
    catch (err) { setError(err.response?.data?.error || 'Failed'); }
  };

  if (loading) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'linear-gradient(135deg,#fdf2f8,#e0f2fe)' }}>
  ...
  const statusColor = s => ({ allocated:'#16a34a', waitlisted:'#d97706', rejected:'#dc2626' }[s]||'#888');
      ...
      <div style={S.banner}>
        <div style={S.bannerInfo}>
          ...
          <div>
            ...
            <div style={S.bannerSub}>{user.roll_number} · {user.department_name} · Year {user.current_year} · CGPA {user.cgpa}</div>
          </div>
        </div>
        <div style={S.bannerStats}>
          <div style={S.bStat}><div style={S.bNum}>{preferences.length}</div><div style={S.bLabel}>Preferences</div></div>
          <div style={S.bStat}><div style={S.bNum}>{courses.length}</div><div style={S.bLabel}>Courses</div></div>
          <div style={S.bStat}>
            <div style={{...S.bNum, color:result.published?'#4ade80':'#fbbf24'}}>{result.published?'Done':'Pending'}</div>
      ...
      <div style={S.body}>
        ...
        {tab==='courses' && (
          <div>
            <div style={S.pageHeader}>
              <div><h2 style={S.pageTitle}>Available Courses</h2><p style={S.pageDesc}>Click Add to include in your preferences</p></div>
              <button onClick={()=>goTab('preferences')} style={S.primaryBtn}>View My Preferences ({preferences.length}) →</button>
            </div>
            <div style={S.courseGrid}>
              {courses.map(course=>{
                ...
                return (
                  <div key={course.id} style={{...S.courseCard, opacity:full?0.65:1}}>
                    <div style={S.courseTop}>
                      <span style={S.codeTag}>{course.code}</span>
                      <span style={{...S.seatPill, background:full?'#fce4e4':'#dcfce7', color:full?'#c0392b':'#15803d'}}>
                    ...
                    <div style={S.courseSchedule}>{course.schedule||'TBD'}</div>
                    <div style={S.deptPill}>{course.department_name}</div>
                    <button onClick={()=>addCourse(course)} disabled={added||full}
                      style={{...S.addBtn, background:added?'#dcfce7':full?'#f5f5f5':'linear-gradient(135deg,#ec4899,#f472b6)',
                        color:added?'#15803d':full?'#aaa':'#fff', cursor:added||full?'default':'pointer',
        ...
        {tab==='preferences' && (
          <div style={{ maxWidth:700 }}>
            <div style={S.pageHeader}>
              <div><h2 style={S.pageTitle}>My Preferences</h2><p style={S.pageDesc}>Rank 1 = top choice. Use arrows to reorder.</p></div>
              <div style={{ display:'flex', gap:'0.5rem' }}>
                <button onClick={()=>goTab('courses')} style={S.secondaryBtn}>← Browse</button>
                {preferences.length>0 && <button onClick={clearAll} style={S.dangerBtn}>Clear All</button>}
              </div>
            </div>
            {preferences.length===0 ? (
              <div style={S.emptyState}>
                <div style={{ fontSize:52 }}>📋</div>
                <div style={{ marginTop:'0.75rem', fontSize:15, color:'#555' }}>No preferences added yet</div>
                <button onClick={()=>goTab('courses')} style={{...S.primaryBtn, marginTop:'1rem'}}>Browse Courses →</button>
              </div>
            ...
            {preferences.length>0 && (
              <>
                <button onClick={handleSubmit} disabled={saving}
                  style={{...S.primaryBtn, width:'100%', marginTop:'1.25rem', padding:'0.8rem', fontSize:15}}>
                  {saving?'Saving...':'✓ Submit Preferences'}
                </button>
                <p style={{ fontSize:12, color:'#aaa', marginTop:'0.5rem', textAlign:'center' }}>
        ...
        {tab==='results' && (
          <div style={{ maxWidth:700 }}>
            <div style={S.pageHeader}>
              <div><h2 style={S.pageTitle}>Allocation Result</h2><p style={S.pageDesc}>Your course allocation for this semester</p></div>
              <button onClick={loadData} style={S.secondaryBtn}>🔄 Refresh</button>
            </div>
            {!result.published ? (
              <div style={S.emptyState}>
                <div style={{ fontSize:52 }}>⏳</div>
                <div style={{ marginTop:'0.75rem', fontSize:15, color:'#555', fontWeight:600 }}>Results not published yet</div>
                <div style={{ fontSize:13, color:'#aaa', marginTop:'0.4rem' }}>Admin needs to run the allocation engine first</div>
                <button onClick={()=>goTab('preferences')} style={{...S.secondaryBtn, marginTop:'1rem'}}>View My Preferences</button>
              </div>
            ) : result.results.length===0 ? (
              <div style={S.emptyState}>
                <div style={{ fontSize:52 }}>😔</div>
                <div style={{ marginTop:'0.75rem', fontSize:15, color:'#555' }}>No allocation found</div>
                <div style={{ fontSize:13, color:'#aaa', marginTop:'0.4rem' }}>Please contact the administrator</div>
              </div>
            ) : result.results.map((r,i)=>(
              <div key={i} style={{...S.resultCard, borderLeft:`4px solid ${statusColor(r.status)}`}}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'1rem' }}>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:700, fontSize:15, color:'#1e293b' }}>{r.code} — {r.name}</div>
                    <div style={{ fontSize:12, color:'#64748b', marginTop:3 }}>{r.credits} credits · {r.instructor}</div>
                    <div style={{ fontSize:11, color:'#94a3b8', fontFamily:'monospace', marginTop:3 }}>{r.schedule}</div>
                    {r.preference_rank && (
                  ...
                  <span style={{ background:`linear-gradient(135deg,${statusColor(r.status)},${statusColor(r.status)}aa)`, color:'#fff', borderRadius:20, padding:'4px 14px', fontSize:12, fontWeight:700, flexShrink:0 }}>
...
const S = {
  ...
  codeTag:   { fontSize:11, fontWeight:700, color:'#ec4899', background:'rgba(236,72,153,0.1)', padding:'2px 8px', borderRadius:6, border:'1px solid rgba(236,72,153,0.2)' },
